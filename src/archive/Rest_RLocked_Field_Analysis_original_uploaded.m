...
% A: planned windows schematic.
nexttile(tl,1);
hold on;
yline(0,'k-'); xline(0,'k--');
shade_window(cfg.win.CFA_ms, [0.85 0.85 0.85]);
...
xlim([-200 600]); ylim([-1 1]);
set(gca,'YTick',[]);
xlabel('Time from R peak (ms)');
title('A. Planned windows');
text(mean(cfg.win.CFA_ms), 0.15, 'CFA', 'HorizontalAlignment','center');
text(mean(cfg.win.RLocked100to200_ms), 0.35, 'Primary 100-200', 'HorizontalAlignment','center');
text(mean(cfg.win.Early_ms), -0.35, 'Adjacent early', 'HorizontalAlignment','center');
text(mean(cfg.win.Late_ms), 0.15, 'Later post-R', 'HorizontalAlignment','center');
box off;
...
fill_between(timeMs, mu-se, mu+se, [0.85 0.85 0.85]); hold on;
plot(timeMs, mu, 'k-', 'LineWidth', 1.8);
xline(0,'k--'); shade_window(cfg.win.CFA_ms, [0.90 0.90 0.90]); shade_window(cfg.win.RLocked100to200_ms, [0.80 0.90 1.00]); shade_window(cfg.win.Early_ms, [0.88 0.84 1.00]); shade_window(cfg.win.Late_ms, [0.90 0.90 0.90]);
xlim([-200 600]); xlabel('Time from R peak (ms)'); ylabel('Global field power (uV)');
title(sprintf('B. Healthy-control R-locked global field power (n=%d)', sum(hc)));
box off;
...
nexttile(tl,5); plot_scalp_map(mapCFA, locs, channels, cl, 'C. HC peri-R -25 to +25 ms', cfg);
nexttile(tl,6); plot_scalp_map(mapRLocked100, locs, channels, cl, 'D. HC primary 100-200 ms', cfg);
nexttile(tl,7); plot_scalp_map(mapEarly, locs, channels, cl, 'E. HC adjacent early 200-300 ms', cfg);
nexttile(tl,8); plot_scalp_map(mapLate, locs, channels, cl, 'F. HC later post-R 300-400 ms', cfg);

sgtitle('Figure 1. Healthy-control passive resting R-locked EEG field is spatially distributed');
export_figure(fig, fullfile(cfg.figDir, 'Figure1_HC_RLocked_Field_Topography.pdf'), cfg);
end
...
tl = tiledlayout(fig, numel(winNames), numel(stages), 'TileSpacing','compact','Padding','compact');
...
for f = 1:numel(mapFields)
    ...
    for g = 1:numel(stages)
        nexttile(tl, (f-1)*numel(stages)+g);
